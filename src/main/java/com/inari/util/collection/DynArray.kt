...
class DynArray<T> private constructor(
    ...
    fun trim() {
        ...
        array = newInstance(typeClass, size) as Array<T?>
        var index = 0
        var i = 0
        while (i < capacity) {
            if (oldArray[i] != null) {
    ...
    companion object {
        ...
        fun <T> of(type: Class<T>, initialCapacity: Int): DynArray<T> {
            return DynArray(type, initialCapacity, 20)
        }

        fun <T> of(type: Class<T>, initialCapacity: Int, grow: Int): DynArray<T> {
            return DynArray(type, initialCapacity, grow)
        }

        fun <T> fromArray(type: Class<T>, array: Array<T>, grow: Int): DynArray<T> {
            val result = DynArray(type, array.size, grow)
        ...
        fun <T> ofAny(type: Class<*>): T {
            @Suppress("UNCHECKED_CAST")
            return DynArray(type, 50, 20) as T
        }

        fun <T> ofAny(type: Class<*>, initialCapacity: Int): T {
            @Suppress("UNCHECKED_CAST")
            return DynArray(type, initialCapacity, 20) as T
        }

        fun <T> ofAny(type: Class<*>, initialCapacity: Int, grow: Int): T {
            @Suppress("UNCHECKED_CAST")
            return DynArray(type, initialCapacity, grow) as T
        }
    }
